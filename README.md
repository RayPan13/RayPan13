...
我是一位注重效能與使用者體驗的前端工程師，擅長 JavaScript 開發。  
能在不依賴框架的情況下打造流暢互動，也能靈活與框架整合，處理大型網站的前端需求。  
擁有多年媒體網站與行銷活動開發經驗，熟悉 GA/GTM 設定、跨裝置相容性與前端架構優化，  
...
## 技能與工具 (Skills & Tools)
- **前端技術**:  
  精通 **HTML5**、**CSS3 (SCSS)**，擅長使用原生 **JavaScript** 開發高效能的前端功能，  
  並具備良好的響應式設計與跨裝置相容性經驗。
- **框架與工具**:  
  熟悉 **Vue.js** 框架，並能靈活選擇使用 **JavaScript** 完成前端需求。  
  熟練使用 **Git** 進行版本控制，並使用 **Vite**、 **NPM** 提升開發效率。
- **數據與分析**:  
  擅長使用 **Google Tag Manager (GTM)** 和 **Google Analytics 4 (GA4)** 進行數據追蹤與分析，  
  並優化用戶行為。熟悉 **Google Ad Manager (GAM)**，可高效管理廣告並優化流程。  
  利用 **Puppeteer** 製作爬蟲收集並整理競品資訊。
- **SEO 優化**:  
